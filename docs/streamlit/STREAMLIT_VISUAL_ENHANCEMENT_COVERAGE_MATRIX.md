...
Remediation, final remediation QA, approval, and merge are complete. Implementation is active on `feat/streamlit-visual-intelligence-enhancement`. Package 1 is committed and pushed at `82c3f66`; Package 2 is committed and pushed at `a60dcbd`. Package 3 is the active current-visual inventory and quarantine-decision foundation.
...
| ML Validation & Evidence | Assignment and method evidence | Preserve governed proof; keep separate from executive story |
...
- Current reconciled status: **13 CONDITIONAL**, **2 EXCLUDED**, **24 IN PROGRESS**, **134 PLANNED**, **31 VERIFIED**
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| SVE-GOV-01 | Activation gate | Do not begin implementation until remediation, final QA, approval, and merge are complete. | Matrix may exist now for continuity; implementation remains deferred. | Remediation merge is present in Git history; dedicated enhancement work began only afterward. | P0 | Git and project governance | VERIFIED |
| SVE-GOV-02 | Dedicated branch | Create a separate Streamlit enhancement branch after remediation merge. | Recommended branch: feat/streamlit-visual-intelligence-enhancement. | Branch feat/streamlit-visual-intelligence-enhancement and remote tracking verified at baseline commit 82c3f66. | P0 | Git and project governance | VERIFIED |
| SVE-GOV-03 | Controlling checklist | Use this matrix as the authoritative scope for the enhancement phase. | No chart or feature is considered complete outside this matrix. | Every row ends VERIFIED, CONDITIONAL, BLOCKED, or EXCLUDED with reason. Current evidence: Package 1 and Package 2 are reconciled; the remaining matrix rows still require package-by-package closure. | P0 | Git and project governance | IN PROGRESS |
| SVE-GOV-04 | Current theme protection | Preserve the current app theme, typography, navigation, and overall visual identity. | Enhance rather than redesign the app. | Before/after screenshots confirm no regression in the approved theme. Current evidence: Theme protection passed for Package 1 and Package 2; remaining pages are not yet reviewed. | P0 | Git and project governance | IN PROGRESS |
| SVE-GOV-05 | Existing chart protection | Audit every current chart before replacing, removing, or duplicating it. | Classify each as KEEP, ENHANCE, REPLACE, MOVE, or RETIRE. | Baseline inventory records decision and reason for every current chart. Current evidence: Existing-chart decisions are complete for Package 1 and Package 2 only; the remaining pages still require inventory. | P0 | Git and project governance | IN PROGRESS |
| SVE-GOV-06 | Evidence rule | Use real project data and verified artifacts only. | No invented metrics, demo values, or unsupported production claims. | Source path and calculation recorded for every visual. Current evidence: Real-evidence rules are verified for Package 1 and Package 2; phase-wide review remains open. | P0 | Git and project governance | IN PROGRESS |
| SVE-GOV-07 | Production truthfulness | Do not show offline validation as live production performance. | Matured-cohort performance remains conditional until real labels exist. | Monitoring page exposes label availability and evidence status. Current evidence: Offline and scenario evidence are labelled truthfully in Package 1 and Package 2; monitoring-page maturation remains open. | P0 | Git and project governance | IN PROGRESS |
| SVE-GOV-08 | No duplicate storytelling | Avoid multiple charts that communicate the same business message. | Each chart must answer a distinct decision question. | Page-level chart-purpose review completed. Current evidence: Completed packages were reviewed for distinct decision purpose; remaining pages are pending. | P0 | Git and project governance | IN PROGRESS |
| SVE-GOV-09 | Package-based changes | Prepare complete, reviewable packages for substantive updates. | Do not ask the user to manually edit many project files. | Package 1 and Package 2 used complete guarded installers with manifests, backups, tests, and no automatic Git writes. | P0 | Git and project governance | VERIFIED |
| SVE-GOV-10 | New-chat continuity | Keep this matrix and the handoff file physically in the repository. | A new chat must read both files before coding. | Matrix, manifest, and handoff are repository files and were captured by the read-only closure audit. | P0 | Git and project governance | VERIFIED |
| SVE-GOV-11 | No premature completion | Do not mark a row complete from code existence alone. | Rendering, data, interaction, explanation, performance, and QA must pass. | Acceptance evidence attached or referenced. Current evidence: Completed packages passed code, calculation, and rendered QA; remaining rows cannot close from code existence alone. | P0 | Git and project governance | IN PROGRESS |
...
| ID | Page / component | Requirement | Baseline / enhancement rule | Acceptance evidence | Priority | Data dependency | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| SVE-AUD-01 | Executive Overview | Inventory KPI cards, executive charts, conclusions, and navigation. | Preserve strong current theme and business story. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/Executive_Overview.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/Executive_Overview.py | VERIFIED |
| SVE-AUD-02 | Visitor Intent Predictor | Inventory single-visitor score, input controls, decision result, and explanations. | Enhance with local explainability and similar-visitor intelligence. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/pages/1_Visitor_Intent_Predictor.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/pages/1_Visitor_Intent_Predictor.py | VERIFIED |
| SVE-AUD-03 | Batch Scoring | Inventory batch upload, score outputs, targeting charts, and downloads. | Enhance campaign scenarios and audience-quality analysis. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/pages/2_Batch_Scoring.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/pages/2_Batch_Scoring.py | VERIFIED |
| SVE-AUD-04 | Model Benchmark Selection | Inventory benchmark, champion, threshold, calibration, and model evidence. | Reuse approved ML Visual Intelligence data and avoid duplicate charts. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/pages/3_Model_Benchmark_Selection.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/pages/3_Model_Benchmark_Selection.py | VERIFIED |
| SVE-AUD-05 | Business KPI Forecasting | Inventory forecast charts, KPI comparisons, diagnostics, and narrative. | Add uncertainty, backtesting, residuals, and scenarios. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/pages/4_Business_KPI_Forecasting.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/pages/4_Business_KPI_Forecasting.py | VERIFIED |
| SVE-AUD-06 | Anomaly and Outlier | Inventory anomaly rate, risk map, tables, and investigation flow. | Add segment comparisons, profiles, and guided investigation. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/pages/5_Anomaly_Outlier.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/pages/5_Anomaly_Outlier.py | VERIFIED |
| SVE-AUD-07 | Monitoring, Drift and Health | Inventory drift, health, prediction, alert, and label-evidence views. | Add trend history and truthful matured-cohort states. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/pages/6_Monitoring_Drift_Health.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/pages/6_Monitoring_Drift_Health.py | VERIFIED |
| SVE-AUD-08 | MLOps Architecture | Inventory current architecture visuals and explanatory content. | Add interactive layered architecture and lifecycle views. | Page screenshot, chart inventory, source mapping, and KEEP/ENHANCE/REPLACE/MOVE/RETIRE decision recorded for app/pages/7_MLOps_Architecture.py; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | app/pages/7_MLOps_Architecture.py | VERIFIED |
| SVE-AUD-09 | MVD Coverage Proof | Inventory course-proof charts and evidence. | Preserve proof; keep it separate from the main executive story. | Package 1 replaced the old proof page with governed ML Validation & Evidence; decision and lineage are documented in Package 1 reconciliation files and commit 82c3f66. | P0 | app/pages/8_MVD_Coverage_Proof.py | VERIFIED |
| SVE-AUD-10 | Champion PR-AUC comparison | Keep or enhance the existing champion comparison if it remains accurate and readable. | Do not duplicate an existing business message. | Champion comparison source, selection logic, and truthfulness correction are documented and tested in Package 1 commit 82c3f66. | P0 | Existing verified project outputs | VERIFIED |
| SVE-AUD-11 | Threshold trade-off | Keep or enhance the existing precision/recall/F1/target-share trade-off. | Do not duplicate an existing business message. | Decision, source, and enhancement gap documented; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | Existing verified project outputs | VERIFIED |
| SVE-AUD-12 | Campaign trade-off | Keep or enhance buyer-capture versus target-quality analysis. | Do not duplicate an existing business message. | Decision, source, and enhancement gap documented; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | Existing verified project outputs | VERIFIED |
| SVE-AUD-13 | Forecast visuals | Keep useful future KPI and unique-visitor forecast views; remove duplicates. | Do not duplicate an existing business message. | Decision, source, and enhancement gap documented; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | Existing verified project outputs | VERIFIED |
| SVE-AUD-14 | Anomaly visuals | Keep useful anomaly-rate-by-segment and purchase-intent risk map views. | Do not duplicate an existing business message. | Decision, source, and enhancement gap documented; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | Existing verified project outputs | VERIFIED |
| SVE-AUD-15 | Clustering proof | Preserve PCA, K-Means, and DBSCAN proof, but separate course proof from business segmentation. | Do not duplicate an existing business message. | PCA, K-Means, DBSCAN, and LOF proof are preserved separately from business segmentation in Package 1 commit 82c3f66. | P0 | Existing verified project outputs | VERIFIED |
| SVE-AUD-16 | Monitoring proof | Preserve useful Grafana and Prometheus evidence without turning Streamlit into a screenshot gallery. | Do not duplicate an existing business message. | Decision, source, and enhancement gap documented; Package 3 source inventory and decisions completed; user-environment screenshots remain pending; Package 3 source inventory and preservation decision verified on 30 June 2026. Automated screenshots were not produced: the local Playwright driver was incompatible with the installed macOS runtime, and both Chrome capture modes timed out. The project owner accepted the documented QA exception to close Package 3; see docs/streamlit/PACKAGE_03_CAPTURE_EXCEPTION.md. | P0 | Existing verified project outputs | VERIFIED |

## UX — Theme, design, usability, and explanation standards

| ID | Page / component | Requirement | Baseline / enhancement rule | Acceptance evidence | Priority | Data dependency | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| SVE-UX-01 | Design tokens | Centralize current theme colors, typography, spacing, borders, chart backgrounds, and semantic states. | Preserve current appearance; standardize extensions. | Shared design tokens and product CSS verified in Package 1 commit 82c3f66 and reused by Package 2. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-02 | Primary chart library | Choose one primary interactive chart library after auditing current usage. | Do not rewrite good existing charts merely to standardize. | Plotly is the primary interactive chart library for the enhanced evidence pages; existing useful visuals were not rewritten without reason. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-03 | KPI card system | Create reusable KPI cards with value, context, delta, status, and tooltip. | Use only where a KPI helps a decision. | Reusable KPI and detail-card components verified in Package 1 and Package 2 rendered reviews. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-04 | Chart header | Give each chart a decision-focused title and concise subtitle. | Avoid generic titles such as 'Chart' or 'Analysis'. | Decision-focused titles and subtitles verified across Package 1 and Package 2 major visuals. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-05 | Interpretation block | Every major visual must include what it shows, how to read it, actual-number finding, conclusion, recommended action, and limitation. | No generic boilerplate or invented conclusions. | Governed interpretation blocks are calculation-backed and covered by focused tests and rendered review. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-06 | Progressive disclosure | Use tabs, expanders, and drill-downs to separate executive and technical detail. | Keep first view calm and decision-oriented. | Tabs, expanders, and calm first-view hierarchy verified in the completed Package 1 and Package 2 pages. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-07 | Accessibility | Use readable text, color-blind-safe distinctions, non-color cues, keyboard-friendly controls, and clear tooltips. | Do not rely on red/green alone. | Accessibility checklist passed. Current evidence: Material visual readability was reviewed for Package 1 and Package 2; full keyboard and accessibility checklist remains open. | P0 | Current .streamlit configuration and shared app styles | IN PROGRESS |
| SVE-UX-08 | Responsive layout | Support common laptop widths without clipping, overlap, or horizontal overflow. | Avoid fixed-width layouts that break on smaller screens. | Responsive screenshots reviewed. Current evidence: Package 1 and Package 2 passed laptop-width visual review; remaining pages are pending. | P0 | Current .streamlit configuration and shared app styles | IN PROGRESS |
| SVE-UX-09 | Empty states | Provide helpful empty, unavailable, and blocked-data states. | Explain what is missing and the next action. | Helpful unavailable and blocked-evidence states are implemented and tested, including forecast and audience evidence handling. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
...
| SVE-UX-11 | Source notes | Show data source, refresh time, and evidence type where relevant. | Distinguish offline, current, simulated, and production evidence. | Source, evidence type, and freshness notes are visible in the completed enhanced pages. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-12 | Number formatting | Use consistent percentages, compact counts, currency, dates, thresholds, and decimals. | Match business meaning and avoid false precision. | Shared formatting utilities and focused calculation tests verify business-safe counts, percentages, thresholds, and currency. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-13 | Visual density | Limit the number of major visuals shown at once. | Prefer one clear message over decorative complexity. | Package 1 multi-tab review and Package 2 executive visual QA confirmed controlled visual density. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-14 | Data tables | Use searchable, sortable, filterable tables with meaningful column names and export controls. | Do not show raw technical columns by default. | Governed evidence tables use meaningful columns and reusable download controls; Package 2 visible executive export remains intentionally excluded. | P0 | Current .streamlit configuration and shared app styles | VERIFIED |
| SVE-UX-15 | Theme regression | Prevent visual changes from breaking the approved current theme. | Theme changes require explicit reason and review. | Before/after visual regression evidence. Current evidence: Theme regression review passed for Package 1 and Package 2; phase-wide regression review remains open. | P0 | Current .streamlit configuration and shared app styles | IN PROGRESS |
...
| SVE-SHARED-09 | Cache data | Use st.cache_data for stable transformed tables and chart inputs. | Cache keys must include relevant filters and source timestamps. | Cache correctness and invalidation tests. Current evidence: Caching exists for completed evidence paths; filter-aware cache correctness and invalidation testing remain open. | P0 | Shared Streamlit utilities and verified derived datasets | IN PROGRESS |
...
| SVE-SHARED-11 | Precompute heavy outputs | Precompute UMAP, SHAP summaries, backtests, drift history, and large aggregation tables. | Do not block page loads with avoidable heavy computation. | Precompute scripts and manifests verified. Current evidence: Package 1 precomputes validation outputs; UMAP, forecast backtests, drift history, and other later-package outputs remain open. | P0 | Shared Streamlit utilities and verified derived datasets | IN PROGRESS |
...
| SVE-SHARED-13 | Sampling policy | Use deterministic sampling for dense scatter plots while preserving totals and distribution context. | Show sample size and full-population totals. | Package 1 uses one deterministic sample with random state 42 and records full/sample row context in the manifest. | P0 | Shared Streamlit utilities and verified derived datasets | VERIFIED |
| SVE-SHARED-14 | Lazy rendering | Render expensive sections only when opened or requested. | Avoid computing every tab at initial load. | Initial page-load timing passes. Current evidence: Progressive disclosure is present, but initial-load timing and explicit lazy-computation evidence remain open. | P0 | Shared Streamlit utilities and verified derived datasets | IN PROGRESS |
...
| SVE-SHARED-16 | Data contracts | Define source path, grain, key, required columns, refresh rule, and fallback for every visual. | No visual may depend on an undocumented table. | Data-contract table complete. Current evidence: Data contracts are complete for Package 1 and Package 2; remaining pages are pending. | P0 | Shared Streamlit utilities and verified derived datasets | IN PROGRESS |
| SVE-SHARED-17 | Artifact manifests | Record generation time, source hashes, row counts, and parameters for precomputed outputs. | Make stale evidence visible. | Package 1 manifest records source hash, row counts, generation time, parameters, selection rules, outputs, and limitations. | P0 | Shared Streamlit utilities and verified derived datasets | VERIFIED |
| SVE-SHARED-18 | Download governance | Exports must contain active filters, timestamp, threshold, source, and column definitions. | Do not export misleading unlabeled files. | Download test and sample file review. Current evidence: Governed downloads exist, but phase-wide active-filter metadata and the accepted Package 2 export omission remain open. | P0 | Shared Streamlit utilities and verified derived datasets | IN PROGRESS |

## EXEC — Executive overview

| ID | Page / component | Requirement | Baseline / enhancement rule | Acceptance evidence | Priority | Data dependency | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| SVE-EXEC-01 | Executive Overview | Enhance current KPI cards with context, evidence type, and concise business meaning. | Preserve strong current cards; improve only where decision value increases. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-02 | Executive Overview | Create an interactive total visitors → scored → eligible → targeted → expected buyers funnel. | Use real counts; simulations must be labeled. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-03 | Executive Overview | Simulate target size, contact cost, expected buyers, value, and uplift assumptions. | Keep assumptions editable and explicit. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-04 | Executive Overview | Show audience reduction versus buyer capture and campaign efficiency. | Reuse verified threshold/campaign evidence where possible. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-05 | Executive Overview | Show composition of the selected target audience by segment, intent tier, and anomaly status. | Avoid pie charts when categories are numerous. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-06 | Executive Overview | Show champion, threshold, validation/holdout evidence, and probability reliability at executive level. | Do not present live production metrics without matured labels. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-07 | Executive Overview | Show a compact forward KPI outlook with uncertainty and scenario context. | Link to full forecasting page. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-08 | Executive Overview | Show current anomaly volume, rate, severity, and key affected segment. | Use real anomaly evidence. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-09 | Executive Overview | Show application, scoring, drift, alerts, and label readiness in one compact strip. | Separate local validation from managed production. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-10 | Executive Overview | Generate a concise decision summary from active filters and actual results. | No generic static paragraph. | Package 2 focused suite: 31 passed; Python compilation passed; user visual QA accepted on 2026-06-30; see PACKAGE_02_FINAL_RECONCILIATION.md. | P1 | Existing verified project outputs | VERIFIED |
| SVE-EXEC-11 | Executive Overview | Add clear links to investigate model, audience, forecasts, anomalies, and monitoring. | Reduce page-search effort. | EXCLUDED from the accepted Package 2 closure: the five investigation links were not visibly available in the rendered page. The user explicitly accepted this omission; revisit only as a later enhancement. | P1 | Existing verified project outputs | EXCLUDED |
| SVE-EXEC-12 | Executive Overview | Provide an exportable filtered executive brief or summary table. | Include timestamp, filters, assumptions, and limitations. | EXCLUDED from the accepted Package 2 closure: the executive brief CSV download was not visibly available in the rendered page. The user explicitly accepted this omission; revisit only as a later enhancement. | P1 | Existing verified project outputs | EXCLUDED |
...
| ID | Page / component | Requirement | Baseline / enhancement rule | Acceptance evidence | Priority | Data dependency | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| SVE-QA-01 | Matrix validation | Validate unique IDs, allowed statuses, required fields, and complete category coverage. | Matrix itself must be testable. | Automated matrix reconciliation tests validate 204 unique IDs, allowed statuses, CSV/Markdown agreement, manifest counts, and package status evidence. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | VERIFIED |
| SVE-QA-02 | Page smoke tests | Every Streamlit page imports and renders without exception. | Use headless tests where practical. | Smoke suite passes. Current evidence: Focused runtime and import checks passed for completed packages; every Streamlit page has not yet been revalidated. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
| SVE-QA-03 | Chart coverage tests | Extend chart-coverage tests to all required visuals and states. | Count alone is insufficient; validate identity and source. | Coverage suite passes. Current evidence: Package 1 and Package 2 chart identities and source coverage are tested; remaining matrix visuals are pending. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
...
| SVE-QA-05 | Calculation tests | Test KPI, funnel, threshold, scenario, drift, forecast, and export calculations. | Use fixed fixtures and reconciliation checks. | Calculation suite passes. Current evidence: Package 1 and Package 2 calculations are tested; remaining journey, drift-history, and later-package calculations are pending. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
| SVE-QA-06 | Data-contract tests | Validate source paths, columns, grain, keys, row counts, and schema hashes. | Fail clearly on stale or incompatible inputs. | Contract suite passes. Current evidence: Completed-package source schemas and evidence contracts are tested; phase-wide contract coverage remains open. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
...
| SVE-QA-09 | Download tests | Validate all exports, metadata, filters, file names, and row counts. | Open sample files during QA. | Download suite passes. Current evidence: Package 1 evidence downloads are covered; all exports are not complete and Package 2 executive export is excluded. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
| SVE-QA-10 | Visual hygiene | Check titles, labels, legends, spacing, clipping, overlap, responsive layout, and theme compatibility. | A visual is incomplete until it renders cleanly. | Automated and manual visual QA passes. Current evidence: Package 1 and Package 2 manual visual QA passed; remaining pages and final combined review remain open. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
| SVE-QA-11 | Accessibility review | Review contrast, color dependence, keyboard use, text alternatives, and table usability. | Fix all material findings. | Accessibility checklist accepted. Current evidence: Readability checks passed for completed packages; the full accessibility checklist remains open. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
| SVE-QA-12 | Truthfulness review | Verify evidence labels, simulated values, offline metrics, and production claims. | No unsupported claim may remain. | Truthfulness audit passes. Current evidence: Truthfulness corrections passed for Package 1 and Package 2; remaining pages require review. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
...
| SVE-QA-16 | QA | Git diff review | Inspect staged files, generated artifacts, comments, and temporary files before each commit. | Do not stage helper packages, local environments, or caches. | Clean reviewed commit history. Package 1 commit 82c3f66 and Package 2 commit a60dcbd were reviewed and pushed; Package 3 is documentation-only inventory work. | P0 | Tests, screenshots, manifests, documentation, and Git evidence | IN PROGRESS |
